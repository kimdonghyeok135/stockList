...
- 문제 : 주식 데이터 호출 시 하나라도 실패하면 전체 실패로 판단하는 이슈 발생
- 문제 해결 : 여러 종목의 시세를 한번에 조회 할 때는 Promise.allSettled로 변경해 응답을 선별적으로 반환하도록 수정.
- 회고 : 병렬 처리 자체보다 더 중요한 것은 실패를 어디까지 전파하고 어떻게 대응할 것인가도 설계에 포함될 수 있다는걸 다시 느낌.

## 트러블 슈팅 2

- 문제 : fetch에 실패한 경우 재시도 기능 구현이 없어, 사용자 UI가 불편하다고 느낌
- 문제 해결 : react-query에서 refetch를 사용하여 해결
- 회고 : 재시도 기능도 화면 설계에 필요하다고 느낌.
