...
export default function Calculator() {
  ...
  const [results, setResults] = useState<ICombinedResult>({ level: null, equal: null, bullet: null }); // 통합 결과 상태
  ...
  const calculateBulletPayment = (p: number, r: number, n: number): IResultBullet => {
    ...
    if (p === 0 || totalPayments === 0) {
        return { monthlyInterestPayment: 0, finalPrincipalPayment: 0, totalInterest: 0, totalPayment: 0 };
    }
                ...
                <div className="flex rounded-lg bg-gray-200 p-1 space-x-1 shadow-inner text-sm">
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-md font-medium transition-all duration-300 ${
                            calculationType === "level" ? 'bg-white text-blue-600 shadow-md' : 'text-gray-600 hover:text-gray-900'
                        }`}
                    >
                        원리금균등
                    </button>
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-md font-medium transition-all duration-300 ${
                            calculationType === "equal" ? 'bg-white text-blue-600 shadow-md' : 'text-gray-600 hover:text-gray-900'
                        }`}
                    >
                        원금균등
                    </button>
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-md font-medium transition-all duration-300 ${
                            calculationType === "bullet" ? 'bg-white text-blue-600 shadow-md' : 'text-gray-600 hover:text-gray-900'
                        }`}
                    >
                        만기일시
                    </button>
            ...
            <div className="space-y-3">
              ...
              {calculationType === "level" && results.level ? (
                  ...
                  <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg border border-blue-200">
                      ...
                      <span id="monthly-payment" className="text-xl font-bold text-blue-600">{formatNumber(results.level.monthlyPayment)} 원</span>
                  </div>
                  ...
                  <>
                      <div className="flex justify-between items-center p-3 bg-red-50 rounded-lg border border-red-200">
                          ...
                          <span id="first-payment" className="text-xl font-bold text-red-600">{formatNumber(results.equal.firstMonthlyPayment)} 원</span>
                      </div>
                      <div className="flex justify-between items-center p-3 bg-red-50 rounded-lg border border-red-200">
                          <span className="text-gray-700 font-medium">마지막 달 상환액:</span>
                          <span id="last-payment" className="text-xl font-bold text-red-600">{formatNumber(results.equal.lastMonthlyPayment)} 원</span>
                      </div>
                  </>
                  ...
                  <>
                      <div className="flex justify-between items-center p-3 bg-indigo-50 rounded-lg border border-indigo-200">
                          ...
                          <span id="monthly-interest-payment" className="text-xl font-bold text-indigo-600">{formatNumber(results.bullet.monthlyInterestPayment)} 원</span>
                      </div>
                      <div className="flex justify-between items-center p-3 bg-indigo-50 rounded-lg border border-indigo-200">
                          <span className="text-gray-700 font-medium">만기일 원금 상환액:</span>
                          <span id="final-principal-payment" className="text-xl font-bold text-indigo-600">{formatNumber(results.bullet.finalPrincipalPayment)} 원</span>
                      </div>
                  </>
              ) : null}
              ...
              <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg border border-green-200">
                ...
                <span id="total-interest" className="text-lg font-bold text-green-600">{formatNumber(currentResult!.totalInterest)} 원</span>
              </div>
              
              {/* 총 상환액 (공통) */}
              <div className="flex justify-between items-center p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                <span className="text-gray-700 font-medium">총 상환액:</span>
                <span id="total-payment" className="text-lg font-bold text-yellow-600">{formatNumber(currentResult!.totalPayment)} 원</span>
              </div>
            </div>
          </div>
        )}
